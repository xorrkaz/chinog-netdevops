...
gitlab_host="http://%%DEV_IP_ADDRESS%"
gitlab_user="root"
gitlab_password="Chinog12345"
gitlab_wait_time=45
...
output=$(curl -s -X POST -H "Authorization: Bearer ${token}" -H "Content-type: application/json" --data-raw "{\"title\": \"Devbox\", \"key\": \"${key}\"}" ${gitlab_host}/api/v4/user/keys)
if [ $? != 0 ]; then
    echo "FAIL!"
    echo "Result = ${output}"
else
